...
Users can request a specific style by name to skip the preview phase entirely. When a user explicitly requests one of these styles, proceed directly to content gathering (if needed) and then generate the presentation in that style.

**Supported named styles:**
- `"Anthropic"` or `"Claude style"` — Warm, minimal, human. See STYLE_PRESETS.md for details.
- Any style name from STYLE_PRESETS.md (e.g., "Neon Cyber", "Paper & Ink", "Swiss Modern")

Example triggers:
- "Make it in Anthropic style"
- "Use the Claude aesthetic"
- "I want the Neon Cyber look"
...
| Mood | Style Options |
|------|---------------|
| Impressed/Confident | "Corporate Elegant", "Dark Executive", "Clean Minimal", "Anthropic" |
| Excited/Energized | "Neon Cyber", "Bold Gradients", "Kinetic Motion" |
| Calm/Focused | "Paper & Ink", "Soft Muted", "Swiss Minimal", "Anthropic" |
